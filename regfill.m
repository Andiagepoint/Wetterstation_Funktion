function [ register_data_hwk_kompakt ] = regfill( register_start, data_struct, prog_scope, prog_detail)
...
%             day_fields = fieldnames(data_struct.(scope_fields{u}));
%             for t = 1:numel(day_fields)
%                 hour_fields = fieldnames(data_struct.(scope_fields{u}).(day_fields{t}));
%                 for s = 1:numel(hour_fields)
%                 data_struct.(scope_fields{u}).(day_fields{t}).(hour_fields{s}) = dec2hex(register_start,4);
